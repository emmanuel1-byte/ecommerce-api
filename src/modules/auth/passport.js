 ...
 *
 * @param {string} clientID - The client ID for the Google OAuth2 application.
 * @param {string} clientSecret - The client secret for the Google OAuth2 application.
...
passport.use(
  new GoogleStrategy(
    ...
    async function (accessToken, refreshToken, profile, cb) {
      try {
        const existingUser = await repository.findUserByEmail(profile.emails[0].value);
        if (!existingUser) {
          const newUser = await repository.create({
            email: profile.emails[0].value,
            role: 'User', fullname: profile.displayName,
